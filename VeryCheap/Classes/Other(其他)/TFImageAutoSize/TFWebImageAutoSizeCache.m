...
@interface TFWebImageAutoSizeCache ()
@property (nonatomic ,strong) NSCache *memCache;
@property (nonnull ,strong) NSFileManager *fileManager;
@end

@implementation TFWebImageAutoSizeCache

+ (TFWebImageAutoSizeCache *)shardCache
{
    static TFWebImageAutoSizeCache *instance = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken,^{
        instance = [[TFWebImageAutoSizeCache alloc] init];
    });
    return instance;
}
...
#pragma mark - TFWebImageAutoSizeCache (private)
...
- (NSData *)dataFromDict:(NSDictionary *)dict
{
    ...
    if (error) {
        TFLog(@"ERROR, faild to get json data");
        return nil;
    }
    return data;
}
...
- (NSString *)baseCacheDirectory
{
    ...
    NSString *path = [pathOfLibrary stringByAppendingPathComponent:@"TFWebImageAutoSizeCache"];
    return path;
...
- (void)createBaseDirectoryAtPath:(NSString *)path
{
    ...
    if (error) {
        TFLog(@"create cache directory failed, error = %@", error);
    } else {
...
- (void)addDoNotBackupAttribute:(NSString *)path
{
    ...
    if (error) {
        TFLog(@"error to set do not backup attribute, error = %@", error);
    }
}
@end
